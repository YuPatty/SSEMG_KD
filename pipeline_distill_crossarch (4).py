...
def run_crossarch_distillation(teacher_cfg, student_cfg, teacher_weights_path, data_root,
                                ...
                                w_feature=0.5, include_relation=False, w_relation=20.0,
                                resp_weights=None, no_kd=False,
                                anneal_schedule='linear', anneal_exp_k=5.0):
    """
    ⚠️ 以下預設值已對齊最終驗證過的生產設定（不是任意值）：
        kd_weight_mode='annealed' : 60-epoch 完整訓練下驗證優於 fixed
        w_feature=0.5             : {0.1,0.3,0.5,0.7} 消融實驗的峰值
        w_relation=20.0           : 原論文預設 0.3 貢獻度<1%（測試無效），
                                     反推校正後才是有意義的權重
    若要直接呼叫此函式（不透過命令列 CLI），也會拿到這組已驗證的預設值，
    不會跟 argparse 那層的預設值不同步。
    resp_weights 若不指定，仍預設全部 1.0（未平衡），因為這是輸入參數而非
    寫死的常數，呼叫端應明確傳入 {"mask":3.5,"mag":5.0,"pha":0.3,"com":2.3}
    才會套用已驗證的平衡權重——這裡沒有偷偷改成非 None 預設值，避免函式
    簽章看起來設了預設但實際數值卻藏在別處，不易察覺。
    ...
    kd_weight_mode: 'fixed'（EDA 論文精神，alpha 固定）或 'annealed'（PreFallKD 精神，
                    alpha 隨 epoch 依 anneal_schedule 曲線遞減）。已驗證：
                    完整 60-epoch 生產訓練下 annealed 優於 fixed。
    anneal_schedule: 'linear'（已驗證，目前生產設定用這個）/'cosine'/'exponential'
                    （新增選項，尚未經消融驗證，見 distill_loss.py 說明）。
    w_feature / include_relation / w_relation: 見 distill_loss.py 的說明，
                    數值都建議做 ablation（例如 w_feature ∈ {0.1, 0.3, 0.5, 0.7}）。
    resp_weights: dict，{"mask":.., "mag":.., "pha":.., "com":..}，用來平衡四個
                    response 分量的梯度貢獻量級（實測發現不加權時 phase 分量會主導，
                    mask/mag 幾乎被蓋過）。預設 None 時函式內部全部設 1.0。
    """
...
if __name__ == '__main__':
    ...
    p.add_argument('--kd_weight_mode', choices=['fixed', 'annealed'], default='annealed',
                    help="GT/KD 權重策略。已驗證：短訓練(30epoch)時 fixed/annealed互有優劣，"
                         "但完整生產訓練(60epoch)下 annealed 全面優於 fixed，"
                         "故此處預設改為 annealed（原本是 fixed，已依實驗結果更新）")
    ...
    p.add_argument('--anneal_schedule', choices=['linear', 'cosine', 'exponential'], default='linear',
                    help="annealed 模式下 alpha 的遞減曲線形狀。linear 是唯一實測驗證過的"
                         "（60-epoch 生產訓練用的就是這個）。cosine/exponential 是新增選項，"
                         "尚未經過消融驗證，導入生產設定前建議先跑至少一組完整長度的對照實驗")
    p.add_argument('--anneal_exp_k', type=float, default=5.0,
                    help="只有 --anneal_schedule exponential 時生效，控制下降速度")
    p.add_argument('--w_feature', type=float, default=0.5,
                    help="已驗證：{0.1,0.3,0.5,0.7} 消融實驗峰值落在 0.5（0.7時反轉變差），"
                         "故預設值由 0.3 更新為 0.5")
